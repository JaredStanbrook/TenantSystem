  ...
  return (
    <>
      <div className="flex flex-col min-h-screen scroll-smooth">
        {/* Sticky Nav - Replaces PageNavContext 
        <nav className="fixed top-0 left-0 z-50 bg-background/80 backdrop-blur-md border-b border-border/40 w-full">
          <div className="container mx-auto flex justify-center py-4 space-x-6">
            {["Rooms", "About", "FAQ"].map((item) => (
              <a
                key={item}
                href={`#${item.toLowerCase()}`}
                className="text-sm md:text-base font-medium px-2 text-foreground hover:text-primary cursor-pointer transition-colors">
                {item}
              </a>
            ))}
          </div>
        ...
        */}
        ...
        <section className="bg-primary p-6 md:p-12">
          <h1 className="text-2xl md:text-5xl font-bold text-secondary mb-8 leading-tight">
            Looking for friendly, easygoing housemates to join me in a beautiful, spacious share
            house in Forrestfield, Perth. Enjoy a relaxed, welcoming vibe, leafy gardens, and plenty
  ...
  <section id="gallery" className="relative h-[80vh] w-full">
    ...
    <div className="relative z-10 h-full flex items-center justify-center">
      ...
      <waitlist-dialog>
        <button
          slot="trigger"
          className="border-2 border-background text-background px-10 py-4 md:py-10 rounded-full text-3xl font-semibold backdrop-blur-lg shadow-xl transition-all duration-300 hover:scale-105 focus:outline-none hover:bg-background/10">
          Join Our House
        </button>
      </waitlist-dialog>
    </div>
        ...
        <div className="bg-secondary text-primary overflow-hidden rounded-lg shadow-2xl mx-4">
          ...
          <div className="p-8 flex flex-col items-center text-left">
            ...
            <div className="space-y-4 text-xl sm:text-2xl font-medium leading-relaxed">
              <p>
                Hi! I'm Jared, a 23-year-old IT student. Two of my good friends are moving soon, and
                I'm keen for some new friendly faces to fill their rooms! I love gardening, cooking,
  ...
  return (
    <section id="faq" className="bg-primary text-primary min-h-[50vh]">
      <div className="grid md:grid-cols-2 gap-8 container mx-auto px-4 py-16">
        <h2 className="text-4xl md:text-5xl text-secondary font-bold mb-4 p-4 leading-tight sticky top-24 self-start">
          Everything you need to know about living here, our house rules, culture, and what to
          expect as a housemate.
        </h2>

        <div className="space-y-4">
          {/* Replaced Radix Accordion with native HTML <details> for no-js functionality */}
          {questions.map((q, i) => (
            <details
              key={i}
              className="group bg-secondary rounded-xl px-2 open:pb-4 transition-all duration-300">
              <summary className="flex justify-between w-full items-center text-left text-primary font-semibold text-xl px-6 py-6 cursor-pointer list-none hover:opacity-90">
                <span>{q.question}</span>
                <div className="flex items-center justify-center w-10 h-10 rounded-lg bg-primary text-secondary shrink-0 transition-transform duration-300 group-open:rotate-45">
                  {/* Plus Icon */}
                  <svg
              ...
              <div className="px-6 text-lg text-primary animate-in slide-in-from-top-2 duration-200">
                {q.answer}
              </div>
            </details>
          ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-10 mb-10 mt-8">
        ...
        <div>
          ...
          <ul className="space-y-2">
            ...
            <li>
              <a href="mailto:jared@stanbrook.me" className="hover:text-primary transition-colors">
                Email
              </a>
            </li>
          </ul>
