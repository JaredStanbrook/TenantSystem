const getPropertyFilterElements = () => {
...
const applyPropertyFilters = () => {
  const { filters, list, search, status, historyToggle } =
    getPropertyFilterElements();
  if (!filters || !list) return;
  const rows = Array.from(list.querySelectorAll("tr[id^='row-']"));
...
export const initPropertyFilters = () => {
  ...
  search?.addEventListener("input", () => applyPropertyFilters());
  status?.addEventListener("change", () => applyPropertyFilters());
