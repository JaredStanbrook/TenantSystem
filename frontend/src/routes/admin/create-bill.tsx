...
import { createBill, getAllBillsQueryOptions } from "@/api/billApi";
...
function CreateBill() {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      const existingBills = await queryClient.ensureQueryData(getAllBillsQueryOptions);
      navigate({ to: "/admin/bill" });
      try {
        // If rent type, use the date range information
        const billData = {
          ...value,
          type: billType,
        };
        ...
        queryClient.setQueryData(getAllBillsQueryOptions.queryKey, {
          ...existingBills,
          bills: [newBill, ...existingBills.bills],
        });
