...
// .wrangler/tmp/bundle-0lJoUg/checked-fetch.js
...
// server/user.ts
var getUser = createMiddleware(async (c2, next) => {
  try {
...
// server/db/index.ts
var dbMiddleware = createMiddleware(
  async (c2, next) => {
...
var MySqlTable = class extends Table {
  /** @internal */
  [(_a95 = entityKind, Table.Symbol.Columns)];
  /** @internal */
  [InlineForeignKeys3] = [];
  /** @internal */
  [Table.Symbol.ExtraConfigBuilder] = void 0;
...
__publicField(MySqlTable, _a95, "MySqlTable");
/** @internal */
...
_a96 = entityKind;
__publicField(ForeignKeyBuilder3, _a96, "MySqlForeignKeyBuilder");
var _a97;
...
_a97 = entityKind;
__publicField(ForeignKey3, _a97, "MySqlForeignKey");
...
_a98 = entityKind;
__publicField(UniqueConstraintBuilder3, _a98, "MySqlUniqueConstraintBuilder");
var _a99;
...
_a99 = entityKind;
__publicField(UniqueOnConstraintBuilder3, _a99, "MySqlUniqueOnConstraintBuilder");
var _a100;
...
_a100 = entityKind;
__publicField(UniqueConstraint3, _a100, "MySqlUniqueConstraint");
...
_a101 = entityKind;
__publicField(MySqlColumnBuilder, _a101, "MySqlColumnBuilder");
var _a102;
...
_a102 = entityKind;
__publicField(MySqlColumn, _a102, "MySqlColumn");
var _a103;
...
_a103 = entityKind;
__publicField(MySqlColumnBuilderWithAutoIncrement, _a103, "MySqlColumnBuilderWithAutoIncrement");
var _a104;
...
_a104 = entityKind;
__publicField(MySqlColumnWithAutoIncrement, _a104, "MySqlColumnWithAutoIncrement");
...
_a105 = entityKind;
__publicField(MySqlCharBuilder, _a105, "MySqlCharBuilder");
var _a106;
...
_a106 = entityKind;
__publicField(MySqlChar, _a106, "MySqlChar");
...
_a107 = entityKind;
__publicField(MySqlVarBinaryBuilder, _a107, "MySqlVarBinaryBuilder");
var _a108;
...
_a108 = entityKind;
__publicField(MySqlVarBinary, _a108, "MySqlVarBinary");
...
_a109 = entityKind;
__publicField(MySqlVarCharBuilder, _a109, "MySqlVarCharBuilder");
var _a110;
...
_a110 = entityKind;
__publicField(MySqlVarChar, _a110, "MySqlVarChar");
...
_a111 = entityKind;
__publicField(PgCharBuilder, _a111, "PgCharBuilder");
var _a112;
...
_a112 = entityKind;
__publicField(PgChar, _a112, "PgChar");
...
_a113 = entityKind;
__publicField(PgUUIDBuilder, _a113, "PgUUIDBuilder");
var _a114;
...
_a114 = entityKind;
__publicField(PgUUID, _a114, "PgUUID");
...
function processCreateParams(params) {
  ...
  const customMap = /* @__PURE__ */ __name((iss, ctx) => {
    ...
    if (typeof ctx.data === "undefined") {
      return { message: (_a117 = message !== null && message !== void 0 ? message : required_error) !== null && _a117 !== void 0 ? _a117 : ctx.defaultError };
    }
...
var ZodType = class {
  ...
  safeParse(data, params) {
    ...
    const ctx = {
      common: {
        issues: [],
        async: (_a117 = params === null || params === void 0 ? void 0 : params.async) !== null && _a117 !== void 0 ? _a117 : false,
...
var ZodString = class extends ZodType {
  ...
  datetime(options) {
    ...
    return this._addCheck({
      ...
      offset: (_a117 = options === null || options === void 0 ? void 0 : options.offset) !== null && _a117 !== void 0 ? _a117 : false,
...
ZodString.create = (params) => {
  ...
  return new ZodString({
    ...
    coerce: (_a117 = params === null || params === void 0 ? void 0 : params.coerce) !== null && _a117 !== void 0 ? _a117 : false,
  ...
  strict(message) {
    ...
    return new ZodObject({
      ...
      ...message !== void 0 ? {
        errorMap: (issue, ctx) => {
          var _a117, _b, _c, _d;
          const defaultError = (_c = (_b = (_a117 = this._def).errorMap) === null || _b === void 0 ? void 0 : _b.call(_a117, issue, ctx).message) !== null && _c !== void 0 ? _c : ctx.defaultError;
  ...
  if (check)
    return ZodAny.create().superRefine((data, ctx) => {
      ...
      if (!check(data)) {
        ...
        const _fatal = (_b = (_a117 = p2.fatal) !== null && _a117 !== void 0 ? _a117 : fatal) !== null && _b !== void 0 ? _b : true;
...
var expense = sqliteTable(
  ...
  (expense2) => {
    return {
      userIdIndex: index("name_idx").on(expense2.userId)
    };
  }
);
var insertExpenseSchema = c(expense, {
...
var selectExpenseSchema = b(expense);

// server/db/schema/tenant.ts
var tenant = sqliteTable("tenant", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  userId: integer("user_id").notNull(),
  givenName: text("given_name").notNull(),
  familyName: text("family_name").notNull()
});
var insertTenantSchema = c(tenant, {
  givenName: z.string(),
  familyName: z.string()
});
var selectTenantSchema = b(tenant);

// server/sharedTypes.ts
var createExpenseSchema = insertExpenseSchema.omit({
  userId: true,
  createdAt: true,
  id: true
});
var createTenantSchema = insertTenantSchema.omit({
  id: true
});

// server/routes/expense.ts
var expenseRoute = new Hono2().use("*", dbMiddleware).get("/", getUser, async (c2) => {
  const user = c2.var.user;
  const expense2 = await c2.var.db.select().from(expense).where(eq(expense.userId, user.id)).limit(100);
  c2.status(200);
  return c2.json({ expense: expense2 });
}).post("/", getUser, zValidator("json", createExpenseSchema), async (c2) => {
  const expense2 = await c2.req.valid("json");
  const user = c2.var.user;
  ...
  const result = await c2.var.db.insert(expense).values(parsedExpense).returning().then((res) => res[0]);
  c2.status(201);
  return c2.json(result);
}).get("/total-spent", getUser, async (c2) => {
  const user = c2.var.user;
  const result = await c2.var.db.select({ total: sum(expense.amount) }).from(expense).where(eq(expense.userId, user.id)).limit(1).then((res) => res[0]);
  return c2.json(result);
}).get("/:id{[0-9]+}", getUser, async (c2) => {
  const id = Number.parseInt(c2.req.param("id"));
  const user = c2.var.user;
  const expense2 = await c2.var.db.select().from(expense).where(and(eq(expense.userId, user.id), eq(expense.id, id))).then((res) => res[0]);
  if (!expense2) {
    return c2.notFound();
  }
  return c2.json({ expense: expense2 });
}).delete("/:id{[0-9]+}", getUser, async (c2) => {
  const id = Number.parseInt(c2.req.param("id"));
  const user = c2.var.user;
  const expense2 = await c2.var.db.delete(expense).where(and(eq(expense.userId, user.id), eq(expense.id, id))).returning().then((res) => res[0]);
  if (!expense2) {
    return c2.notFound();
  }
  return c2.json({ expense: expense2 });
...
var tenantRoute = new Hono2().use("*", dbMiddleware).get("/", getUser, async (c2) => {
  const user = c2.var.user;
  const tenants = await c2.var.db.select().from(tenant).where(eq(tenant.userId, user.id)).limit(10);
  if (tenants.length === 0) {
    c2.status(404);
    return c2.json({ error: "No tenants found for the specified user ID." });
  }
  c2.status(200);
  return c2.json({ tenants });
}).post("/", zValidator("json", createTenantSchema), async (c2) => {
  const tenant2 = await c2.req.valid("json");
  const validatedTenant = insertTenantSchema.parse({
    ...tenant2
  });
  const result = await c2.var.db.insert(tenant).values(validatedTenant).returning().then((res) => res[0]);
  c2.status(201);
  return c2.json(result);
}).get("/:id{[0-9]+}", getUser, async (c2) => {
  const id = Number.parseInt(c2.req.param("id"));
  const user = c2.var.user;
  const tenant2 = await c2.var.db.select().from(tenant).where(eq(tenant.id, id));
  if (!tenant2) {
    return c2.notFound();
  }
  c2.status(200);
  return c2.json({ tenant: tenant2[0] });
}).delete("/:id{[0-9]+}", getUser, async (c2) => {
  const id = Number.parseInt(c2.req.param("id"));
  const user = c2.var.user;
  const tenant2 = await c2.var.db.delete(tenant).where(eq(tenant.id, id)).returning().then((res) => res[0]);
  if (!tenant2) {
    return c2.notFound();
  }
  return c2.json({ tenant: tenant2 });
});

// server/routes/auth.ts
var authRoute = new Hono2().get(
  "/me",
  getUser,
  async (c2) => {
    const user = c2.var.user;
...
// server/index.ts
var app = new Hono2().use("*", logger()).use("*", cors()).get("/*", async (c2) => {
  const object = await c2.env.R2.get(c2.req.path.slice(1));
  if (object != null) {
    c2.status(201);
    return c2.body(object?.body);
  } else {
    return c2.env.ASSETS.fetch(c2.req.raw);
  }
}).basePath("/api").route("/auth", authRoute).route("/tenant", tenantRoute).route("/expense", expenseRoute);
app.onError((err, c2) => {
  console.error(err);
...
// .wrangler/tmp/bundle-0lJoUg/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-0lJoUg/middleware-loader.entry.ts
