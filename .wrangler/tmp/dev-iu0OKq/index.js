...
// .wrangler/tmp/bundle-j7D5bI/checked-fetch.js
...
var getUser = createMiddleware(async (c2, next) => {
  try {
    const user = { id: "admin", given_name: "admin", family_name: "admin" };
    c2.set("user", user);
    await next();
  } catch (e) {
    console.error(e);
...
var expenseRoute = new Hono2().use("*", dbMiddleware).get("/", getUser, async (c2) => {
  ...
  const expense2 = await c2.var.db.select().from(expense).where(eq(expense.userId, user.id)).orderBy(desc(expense.createdAt)).limit(100);
...
// .wrangler/tmp/bundle-j7D5bI/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-j7D5bI/middleware-loader.entry.ts
