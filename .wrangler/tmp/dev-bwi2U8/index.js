...
// .wrangler/tmp/bundle-QfJeDU/checked-fetch.js
...
var getUser = createMiddleware(async (c2, next) => {
  try {
    const user2 = { id: "admin", given_name: "admin", family_name: "admin" };
    c2.set("user", user2);
    await next();
  } catch (e) {
    console.error(e);
...
var expenseRoute = new Hono2().use("*", dbMiddleware).get("/", getUser, async (c2) => {
  ...
  const expense2 = await c2.var.db.select().from(expense).where(eq(expense.userId, user2.id)).orderBy(desc(expense.createdAt)).limit(100);
  ...
  const totalSpent = result ? result.total : 0;
  return c2.json(result);
...
var tenantRoute = new Hono2().use("*", dbMiddleware).get("/", getUser, async (c2) => {
  const tenant2 = await c2.var.db.select().from(tenant).limit(10);
  ...
  const id = Number.parseInt(c2.req.param("id"));
  const tenant2 = await c2.var.db.select().from(tenant).where(eq(tenant.id, id)).then((res) => res[0]);
  if (!tenant2) {
    return c2.notFound();
  }
  return c2.json({ tenant: tenant2 });
...
var authRoute = new Hono2().get("/me", getUser, async (c2) => {
  const user2 = c2.var.user;
  return c2.json({ user: user2 });
});

// server/index.ts
var app = new Hono2().use("*", logger()).basePath("/api").route("/tenant", tenantRoute).route("/expense", expenseRoute).route("/", authRoute);
...
// .wrangler/tmp/bundle-QfJeDU/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-QfJeDU/middleware-loader.entry.ts
